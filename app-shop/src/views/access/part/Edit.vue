  ...
  <el-dialog title="修改菜单&权限" :visible.sync="dialogVisible" @close="dialogFormVisible" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form size="small" :model="formData" :rules="formRules" ref="form">
      ...
      <el-form-item label="路由地址" prop="path" :label-width="formLabelWidth">
        <el-input v-model="formData.path" autocomplete="off" placeholder="请输入路由地址"></el-input>
        <p>例如：/access/index</p>
      </el-form-item>
      <!--组件名-->
      <el-form-item label="组件名" prop="views" :label-width="formLabelWidth">
        <el-input v-model="formData.views" autocomplete="off" placeholder="请输入组件文件路径"></el-input>
        <p>提示：对应前端给的路径+组件名称，例如：access/Index</p>
      </el-form-item>
      <!--图标-->
