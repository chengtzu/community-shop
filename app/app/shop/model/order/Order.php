...
class Order extends OrderModel
{
    ...
    public function delivery($data)
    {
        ...
        foreach ($orderList as $key => $item){
            $updateList[$key]['order_id'] = $item['order_id'];
            if($item['delivery_type'] == DeliveryTypeEnum::EXPRESS){
                $updateList[$key]['express_id'] = $data['express_id'];
                $updateList[$key]['express_no'] = $data['express_no'];
            }
            // var_dump($item);die;
        }
        // 整理更新的数据
        ...
        // if($this['order_id'] == DeliveryTypeEnum::EXPRESS){
    ...
    private function verifyDelivery($orderList)
    {
        foreach ($orderList as $order) {
            // var_dump(DeliveryTypeEnum::EXPRESS);die;
            if (
                $order['pay_status']['value'] != 20
                || $order['delivery_type']['value'] != DeliveryTypeEnum::STORE
