...
class Order extends Controller
{
    ...
    public function buy()
    {
        ...
        if($this->PayDebug){
            $orderres = OrderModel::where('order_id',$order_id)->field('order_no')->find();

            $successres = new \app\api\service\order\paysuccess\type\MasterPaySuccessService($orderres->order_no);
            $successres->onPaySuccess(99);
            // 返回结算信息
            return $this->renderSuccess(['success' => '支付成功'], [
                'order_id' => $order_id,   // 订单id
                'pay_type' => 99,  // 支付方式
                'payment' => '',               // 微信支付参数
                'template_arr' => [],    // 小程序申请的消息模板id
            ]);
        }
